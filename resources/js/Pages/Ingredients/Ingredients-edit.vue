<template>
  <Head>
  <title>{{ingredient.name}} - Edit Ingredient | NutriStudents K-12</title>
  <meta name="description" content="Edit Ingredient">
  </Head>
  <default-layout>
 ...
 <form action="#" @submit.prevent="false">
    ...
    <PageTitleLayout>
      <div class="lg:mb-0 mb-4 w-4/6">
        <h1 class="heading-color 2xl:text-2xl text-xl font-bold">Ingredient: {{ingredient.name}}</h1>
      </div>
      <div class="w-full flex lg:flex-nowrap flex-wrap md:justify-end items-center">
         ...
         <button  type="button" @click="ingredient_edit_save_script" class="menu1-background-color w-full inline-flex justify-center rounded-md font-bold px-4 py-3.5 uppercase button-color text-white  focus:outline-none sm:ml-3 sm:w-40 mt-3 sm:mt-0">Update</button>     
    ...
    <label class="font-bold text-gray-700 2xl:text-lg text-base">Ingredient Name*</label>
     <div>
      <input v-if="ingredient.is_tenant_admin" type="text" id="ing_name" name="ing_name" v-model="form.ing_name"  class="2xl:text-base text-sm text-gray-400 xl:mt-0 mt-4 2xl:w-80 xl:w-80 w-60 shadow-sm focus:border-none block sm:text-sm border-gray-300  rounded-lg select-menu-height input-background">
     <input v-else readonly type="text" id="ing_name" name="ing_name" :value="form.ing_name"  class="2xl:text-base text-sm text-gray-400 xl:mt-0 mt-4 2xl:w-80 xl:w-80 w-60 shadow-sm focus:border-none block sm:text-sm border-gray-300  rounded-lg select-menu-height input-background">
...
<path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
 ...
 <div class="w-full flex lg:flex-nowrap flex-wrap justify-end items-center  mb-6  px-10">
         ...
         <button  type="button" @click="ingredient_edit_save_script" class="menu1-background-color w-full inline-flex justify-center rounded-md font-bold px-4 py-2 uppercase button-color text-white  focus:outline-none sm:ml-3 sm:w-40 mt-3  sm:mt-0">Update</button>
  ...
  <template #footer>
    <div class="flex justify-end items-center">
      ...
      <form  v-if="!error_product_array.is_tenant_admin" target="_blank"  :action="route('product_copy', error_product_array.id)" method="post">
        ...
        <button type="submit"  class="inline-flex justify-center rounded-md font-bold px-4 py-3.5 uppercase bg-red-500 text-white focus:outline-none text-base" 
        > Duplicate & Edit Product </button>
      </form>
    </div>
  </template>
</jet-dialog-modal>
...
export default {
  props: ['ingredient','ing_products','avg_products'],
  components: {
    DefaultLayout,
    Welcome,
    BreadcrumLayout,
  ...
  data(){
    return{
      ...
      form: this.$inertia.form({
                      ing_name: this.ingredient.name,
                      ing_prefer_product: this.ingredient.prefer_product_id,
                      attached_products:[],
                    })
    }
  },
  ...
  methods:{
    ...
    productSearchScriptModal(){
            ...
            axios.post('/ingredient-product-search-modal',{term:this.search_term,ex_ids:exist_products, is_all : this.is_all})
                    .then((response)=>{
    ...
    ingredient_edit_save_script() {
              this.form.attached_products = this.ing_products;
              this.form.post(this.route('ingredients-edit-save', this.ingredient.id), {

                   onSuccess: () => {
                          this.$toast.success('Ingredient has been updated successfully!');
                    },
              });
          },
