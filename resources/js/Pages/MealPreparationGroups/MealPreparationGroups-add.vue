      ...
      <PageTitleLayout>
        <div class="lg:mb-0 mb-4 w-6/12">
          <h1 class="heading-color xl:text-4xl sm:text-2xl text-base font-bold">
            Add New Meal Preparation Group
          </h1>
        </div>
        <div
...
export default {
  ...
  methods: {
    ...
    menu_creation_add_save_script() {
      this.form.post(this.route("meal-preparation-groups-add-save"), {
        onSuccess: () => {
          this.$toast.success("Meal Preparation Group added successfully!");
        },
      });
    },
    addRow(offering_id){
      var index = this.form.siteOfferings.findIndex(x => x.id === offering_id.id);
      let ssoid = this.searched_site_offerings.findIndex(x => x.id === offering_id.id);
      if (index > -1) {
        this.form.siteOfferings.splice(index, 1);
      }
      console.log("this.searched_site_offerings", this.searched_site_offerings);
      if(ssoid > -1){
    ...
    matchOffer(offering_id){
      if(this.form.siteOfferings.length == 0){
        return true;
      }
      var index = this.form.siteOfferings.findIndex(x => x.id === offering_id);
      if (index > -1) {
        return false;
      } else {
        return true;
