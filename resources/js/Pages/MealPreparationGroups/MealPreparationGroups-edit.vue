<template>
  <Head>
    <title>Update Group | NutriStudents K-12</title>
  ...
  <default-layout>
    <BreadcrumLayout>
      <div class="flex">
        ...
        <inertia-link :href="route('meal-preparation-groups')" class="link-color"
          ><U>Meal Preparation Group</U></inertia-link
        >
      </div>
    </BreadcrumLayout>
    ...
    <form action="#" enctype="multipart/form-data">
      <PageTitleLayout>
        ...
        >
          ...
          >
            <inertia-link :href="route('meal-preparation-groups')" class="
              w-full
              inline-flex
              justify-center
              rounded-md
            ...
            >
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody>
                  ...
                  <offering-component
                    v-for="(rec_offering, index) in form.siteOfferings"
  ...
  >
    <template #title>
      <div class="flex justify-between items-center">
        Add Site Offerings to the Meal Preparation Group
                ...
                <tbody>
                  ...
                  >
                    ...
                    >
                      ...
                      <button type="button" @click="addRow(offer)">
...
import OfferingComponent from "@/Pages/MealPreparationGroups/Offering";
...
export default {
  ...
  data() {
    return {
      ...
      form: this.$inertia.form({
        ...
        meal_type_id: this.group.meal_type_id,
        // day_id: this.group.day_id,
        // age_grade_offering_id: this.group.age_grade_offering_id,
        // guideline_id: this.group.guideline_id,
        siteOfferings: this.offering_array,
      }),
    };
  },
  mounted(){
    // if(this.form.meal_type_id){
    ...
    // }
    // if(this.offering_array){
    //   this.offering_array.forEach((value,key) => {
    //     this.form.siteOfferings.push(value.id);
    //   });
    // }
  },
  ...
  methods: {
    ...
    addModalOpen() {
      ...
      this.search_term = "";
      axios
      .post("/meal-preparation-groups-search-site-offerings",{form: this.form})
      .then((response) => {
    ...
    menu_creation_add_save_script() {
      this.form.post(this.route("meal-preparation-groups-edit-save",this.group.id), {
        onSuccess: () => {
          this.$toast.success("Meal Preparation Group updated successfully!");
        },
      });
    },
    addRow(offering_id){
      var index = this.form.siteOfferings.indexOf(offering_id);
      let ssoid = this.searched_site_offerings.findIndex(x => x.id === offering_id.id);
      if (index > -1) {
        this.form.siteOfferings.splice(index, 1);
      }
      if(ssoid > -1){
        this.searched_site_offerings.splice(ssoid, 1);
      }
