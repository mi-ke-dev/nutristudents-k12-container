<template>
  <Head>
  <title>{{site.name}} - Edit Site | NutriStudents K-12</title>
  <meta name="description" content="Edit Site">
  </Head>
  <default-layout>
      ...
      <form action="#" @submit.prevent="false">
        ...
        <PageTitleLayout>
          <div class="lg:mb-0 mb-4 w-4/6">
            <h1 class="heading-color  2xl:text-2xl text-xl font-bold">{{site.name}}</h1>
          </div>
          <div class="w-full flex lg:flex-nowrap flex-wrap md:justify-end items-center">
            ...
            <button type="button" @click="site_edit_save_script" class="menu1-background-color w-full inline-flex justify-center rounded-md font-bold px-4 py-3.5 uppercase button-color text-white  focus:outline-none sm:ml-3 sm:w-40 mt-3 sm:mt-0">Update</button>
  ...
  <div class="w-full flex lg:flex-nowrap flex-wrap justify-end items-center mb-6 px-10 mt-6">
    ...
    <button  type="button" @click="site_edit_save_script" class="menu1-background-color w-full inline-flex justify-center rounded-md font-bold px-4 py-2 uppercase button-color text-white  focus:outline-none sm:ml-3 sm:w-40 mt-3  sm:mt-0">Update</button>
...
<jet-dialog-modal :show="addOfferingModal || editOfferingModal" @close="addOfferingModal = false, editOfferingModal = false">
 <template #title>
  ...
  <jet-secondary-button v-if="editOfferingModal" @click="addSiteOfferingEditModalProcess()" class="ml-4">Update Offering</jet-secondary-button>
  <jet-secondary-button @click="addOfferingModal = false, editOfferingModal = false" class="ml-4">Close</jet-secondary-button>
  </template>
</jet-dialog-modal>
...
<script>
  ...
  export default {
    props:['site','site_users_data','site_offerings_data', 'offering_menuadmins_user','offering_menuadmins_group','offering_orderadmins_user','offering_orderadmins_group', 'offering_sites', 'mealtype_data', 'agerange_data','days_data'],
    components: {
      DefaultLayout,
      Welcome,
      BreadcrumLayout,
    ...
    data(){
      ...
      site_users:this.site_users_data,
      site_offerings:this.site_offerings_data,
    ...
    this.site_users_data.forEach((value,index) => {
    ...
    methods:{
      ...
      site_edit_save_script(){
        this.form.attached_users = this.site_users;
        this.form.attached_offerings = this.site_offerings;
        
        this.form.post(this.route('edit-site-save',this.site.id), {
          onSuccess: () => {    
            this.$toast.success('Site has been updated successfully!');
          },
        });
      },
    ...
    addSiteOfferingDeleteScript(index){
      if(confirm("Are you sure to delete this offering!")){
        if(this.site_offerings[index].id != ""){
          this.form.delete_offerings.push(this.site_offerings[index].id);
        }
    ...
    if(this.offering_name == "" || this.offering_guideline.length == 0 || this.offering_menu_admin.length == 0 || this.offering_order_admin.length == 0 || this.offering_prep_site.length == 0 || this.offering_cook_site.length == 0){
    this.offering_validation = true;
    }else{
