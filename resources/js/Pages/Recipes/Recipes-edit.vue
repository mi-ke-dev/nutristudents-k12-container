<template>
  <Head>
    <title>{{ recipe.name }} - Edit Recipe | NutriStudents K-12</title>
  ...
  <default-layout>
    ...
    <form action="#" @submit.prevent="false" enctype="multipart/form-data">
      <PageTitleLayout>
        ...
        >
          ...
          <button
            v-if="recipe.is_tenant_admin"
            type="button"
            @click="recipe_edit_save_script"
            class="
        ...
        >
          <div class="sm:w-2/5 w-full">
            ...
            <div class="xl:grid xl:gap-1 xl:grid-cols-3 block pl-5 pt-6">
              <label class="font-bold text-gray-700">Recipe Image s</label>
              ...
              <div class="w-full">
                ...
                <div v-else-if="recipeimage" class="relative w-20 mt-2">
                  <img
                    :src="recipeimage"
                    class="xl:mt-0 mt-4"
                    width="100"
                  ...
                  />
                  <i
                    class="fa fa-times absolute top-0 right-0 cursor-pointer"
                    aria-hidden="true"
                    @click="removeRecipeImage"
                  ></i>
                </div>
                <div v-else-if="recipe_image_org" class="relative w-20 mt-2">
                  <img
                    :src="$page.props.storage_image_url + recipe_image_org"
                    class="xl:mt-0 mt-4"
                    width="100"
                    height="100"
                  />
                ...
                <div v-else class="relative w-20 mt-2">
                  <img
                    :src="$page.props.asset_url + '/images/no-image-found.jpg'"
                    class="xl:mt-0 mt-4"
                    width="100"
                    height="100"
                  />
            ...
            <div class="xl:grid xl:gap-1 xl:grid-cols-3 block pl-5 pt-6 items-center">
                ...
                <div>
                <input type="checkbox" id="recipe_portion" name="recipe_portion" v-model="form.is_favorite" v-bind:true-value="true"
                      >
                </div> 
            </div>
          </div>
    ...
    <label class="font-bold text-gray-700">Add HACCP</label>
    </div> -->
              ...
              <div class="relative focus:outline-none xl:w-52 w-72 mr-3">
                <select
                ...
                >
                  ...
                  <option
                    v-if="selected_haccp_names"
                    v-for="haccp_name in selected_haccp_names"
                    :value="haccp_name.id"
                  >
                    {{ haccp_name.name }}
                  </option>
      ...
      >
        ...
        <a href="javascript:void(0)" v-if="recipe.is_tenant_admin" @click="addIngredientRecipeModalOpen">
          <svg
            xmlns="http://www.w3.org/2000/svg"
...
export default {
  ...
  data() {
    return {
      rec_ingredients: this.recipe_ingredients,
      ing_prefer_products: this.prefer_products,
      calculated_totals_sum: [],

      addIngredientsModal: false,
      recipeimage: "",
      recipe_image_org: this.recipe.photo_store_path,
      search_term: "",
      ...
      form: this.$inertia.form({
        ...
        simplified_name:this.recipe.simplified_name,
        recipe_category: this.recipe.category,

        recipe_portion: this.recipe.portion,

        recipe_serving: this.recipe.serving_size,
        recipe_serving_unit: this.recipe.serving_size_uom_id,

        recipe_image: "",
        recipe_edit_image: this.recipe.photo_store_path,

        recipe_instructions: this.recipe.cooking_instructions,
        recipe_haccp_type: this.selected_haccp_type[0],
        recipe_haccp_id: this.recipe.haccp_id,
        is_favorite: (this.recipe.is_favorite)?true:false,
  ...
  mounted() {
    var quill_editor_set = document.querySelector("#quill_editor_instructions");
    quill_editor_set.children[0].innerHTML = this.recipe.cooking_instructions;
  ...
  methods: {
    ...
    recipe_edit_save_script() {
      ...
      if (
        ...
        this.form.recipe_instructions = quill_editor_get.children[0].innerHTML;
      } else {
      ...
      this.form.post(this.route("recipes-edit-save", this.recipe.id), {
        onSuccess: () => {
