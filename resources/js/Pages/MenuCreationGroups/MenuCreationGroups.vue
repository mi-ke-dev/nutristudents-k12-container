<template>
  <Head>
  <title>Menu Creation Group | NutriStudents K-12</title>
  ...
  <default-layout>
    ...
    <PageTitleLayout>
      <div class="lg:mb-0 mb-4">
        <h1 class="heading-color xl:text-4xl text-2xl font-bold">Menu Creation Group</h1>
      </div>
      <div class="w-full flex lg:flex-nowrap flex-wrap justify-end items-center">
        <div class="relative focus:outline-none lg:w-44 sm:w-1/2 w-full">
          <select id="search_meal_type" name="search_meal_type" autocomplete="search_meal_type"
            class="shadow-sm focus:border-none block sm:text-sm border-gray-300 rounded-lg w-full select-menu-height" @change="onChangeFunction" >
        ...
        <div class="ml-5 relative focus:outline-none lg:w-44 sm:w-1/2 w-full">
          <select id="search_offerings" name="search_offerings" autocomplete="search_offerings"
            class="shadow-sm focus:border-none block sm:text-sm border-gray-300 rounded-lg w-full select-menu-height" @change="onChangeFunction" >
            <option value="">Age Range</option>
            <option v-for="category in ageGradeOfferings" :value="category.id">{{category.name}}</option>
          </select>
          <div class="absolute flex justify-center items-center searching-background-color select-menu-height w-9 text-white top-0 right-0 rounded-lg rounded-tl-none rounded-bl-none pointer-events-none"><i class="fa fa-sort-desc" aria-hidden="true"></i></div>
        </div>
        
        <div class="ml-5 relative focus:outline-none lg:w-44 sm:w-1/2 w-full">
          <select id="search_day" name="search_day" autocomplete="search_day"
            class="shadow-sm focus:border-none block sm:text-sm border-gray-300 rounded-lg w-full select-menu-height" @change="onChangeFunction" >
            <option value="">Days</option>
            <option v-for="category in days" :value="category.id">{{category.name}}</option>
          </select>
          <div class="absolute flex justify-center items-center searching-background-color select-menu-height w-9 text-white top-0 right-0 rounded-lg rounded-tl-none rounded-bl-none pointer-events-none"><i class="fa fa-sort-desc" aria-hidden="true"></i></div>
        </div>
        ...
        <inertia-link :href="route('menu-creation-groups.add')" class="button-color text-white font-bold py-3 xl:px-4 px-2 lg:ml-6 sm:ml-6 ml-6 rounded lg:mt-0 sm:mt-0 mt-10">
          CREATE NEW GROUP
          </inertia-link>
      </div>
    </PageTitleLayout>
          ...
          <tbody>
            ...
            <tr v-for="(group,index) in groups.data" :class="{'bg-white':index%2 == 0, 'tr-background-color':index%2 != 0}">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-base text-color td-background-color">
                  <div class="tooltip">
                      <div class="trim_text">{{ (group.age_grade)?group.age_grade.name : ''}}</div>
                    </div>
                </td>
                ...
                <td class="flex justify-center w-24 padding-top-icon text-center align-bottom h-full items-end pt-4">
                  {{group.guideline.name}}
                </td> 
                <td class="px-6 py-4 whitespace-nowrap  id-color text-xl  font-extrabold">
                  
                    <inertia-link :href="route('menu-creation-groups-edit', { id: group.id })" class="">
...
export default {
  ...
  methods:{
        creation_delete_script(rid) {
          ...
          this.form.post(this.route('group-delete', rid), {
                onSuccess: () => {
                  this.$toast.success('Menu creation group deleted successfully!');
                  setTimeout(function(){
        ...
        groupSearchData(search){
            if(search == ""){
              ...
              axios.post('/search-filter',{type:'search',term:search})
              .then((response)=>{
        ...
        onChangeFunction() {  
           axios.post('/search-filter',{type:'filter',term:this.search,meal:this.search_meal_type,day:this.search_day,offrings:this.search_offerings})
          .then((response)=>{
              this.groups = response.data;
